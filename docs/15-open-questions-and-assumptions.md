...
| `A54` | **Utilisation target 85 %**, **downtime target ≤ 10 %** | A world-class-adjacent utilisation target distinct from the more conservative first-year OEE target (`A28`); 10% unplanned+planned downtime is the complement of a healthy availability factor | Executive Dashboard KPI row |
| `A30` | Machine flagged **under-utilised below 60 %** | Consistent with typical baseline OEE | Alert rule / dashboard |
| `A52` | Machine **suggested as a bottleneck at ≥ 90 %** rolling utilisation | The pragmatic, defensible reading of "constrains overall throughput" without a full theory-of-constraints/routing model this project has no data to support yet — a starting point a Plant Manager applies deliberately and can always override by hand | `fmes.utilization.service.BOTTLENECK_THRESHOLD_PCT` |
